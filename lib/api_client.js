...
(function() {
  ...
  ApiClient = (function() {
    ...
    ApiClient.prototype.get = function(params, headers, cb) {
      ...
      start = new Date;
      if (cb) {
        request_opts.callback = function(err, body, headers) {
          var delta, stop;
          stop = new Date;
          delta = stop.getTime() - start.getTime();
          if (ApiClient.logger) {
            ApiClient.logger.info("[APICLIENT] request stop: " + request_opts.uri + ", delta: " + delta);
          }
          return cb(err, body, headers);
        };
      }
